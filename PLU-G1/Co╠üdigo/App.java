...
public class App {
    ...
    */
    ...
    static void localizarPedidosPremium() {
        cabecalho();
        Double valorCorte = lerOpcao("Informe o valor de corte: ", Double.class);
        ...
        int indice = buscaBinariaPorValor(pedidosOrdenadosPorValor, valorCorte);

        System.out.println("Pedidos premium (valor final >=  " + String.format("%.2f", valorCorte) + "):");
        boolean encontrou = false;
        for (int i = indice; i < quantPedidos; i++) {
            Pedido p = pedidosOrdenadosPorValor[i];
            if (p.valorFinal() >= valorCorte) {
        ...
        if (!encontrou) {
            System.out.println("Nenhum pedido encontrado com valor final >= " + String.format("%.2f", valorCorte));
        }
    }

    /**
     * Busca binária para encontrar o primeiro índice onde o valor final do pedido >= valorCorte.
     * Assume que o vetor está ordenado por valorFinal crescente.
     */
    private static int buscaBinariaPorValor(Pedido[] pedidos, double valorCorte) {
        int esquerda = 0;
        int direita = pedidos.length - 1;
        int resultado = pedidos.length; // Se nenhum, retorna length

        while (esquerda <= direita) {
            int meio = esquerda + (direita - esquerda) / 2;
            if (pedidos[meio].valorFinal() >= valorCorte) {
                resultado = meio;
                direita = meio - 1;
            } else {
                esquerda = meio + 1;
